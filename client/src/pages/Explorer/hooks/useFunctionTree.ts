...
export function useFunctionTree() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                ...
                const data = await fetchFunctions(token);
                if (!cancelled) {
    ...
    const handleSearch = useCallback((query: string) => {
        ...
        searchTimer.current = setTimeout(async () => {
            try {
                const token = await getApiAccessToken();
                if (!token) return;
                const data = await fetchFunctions(token, undefined, query);
    ...
    const loadChildren = useCallback(async (parentId: string, parentLevel: number) => {
        ...
        try {
            const token = await getApiAccessToken();
            if (!token) return;
            const data = await fetchFunctions(token, parentId);
