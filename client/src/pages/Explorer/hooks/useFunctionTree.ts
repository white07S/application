...
import { fetchFunctions, ApiTreeNode } from '../api/explorerApi';
...
export function useFunctionTree(asOfDate: string) {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                ...
                const data = await fetchFunctions(token, asOfDate);
                if (!cancelled) {
                    setTreeNodes(data.nodes.map((n) => apiToTreeNode(n, 0)));
                    setDateWarning(data.date_warning || null);
                }
            } catch (err: any) {
                if (!cancelled) setError(err.message || 'Failed to load functions');
            } finally {
    ...
    const handleSearch = useCallback((query: string) => {
        ...
        searchTimer.current = setTimeout(async () => {
            try {
                const token = await getApiAccessToken();
                if (!token) return;
                const data = await fetchFunctions(token, asOfDate, undefined, query);
                setSearchResults(data.nodes.map((n) => apiToTreeNode(n, 0)));
            } catch (err: any) {
                console.error('Function search failed:', err);
    ...
    const loadChildren = useCallback(async (parentId: string, parentLevel: number) => {
        ...
        try {
            const token = await getApiAccessToken();
            if (!token) return;
            const data = await fetchFunctions(token, asOfDate, parentId);
