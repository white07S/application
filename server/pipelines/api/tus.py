...
async def _complete_upload(tus_upload: TusUpload, db: Session) -> None:
    ...
    # All files are complete! Create the UploadBatch and run validation
    logger.info(
    ...
    batch_path = storage.get_upload_batch_path(upload_id, tus_upload.data_type)
    ...
    data_source_id = upload_tracker.get_data_source_id(db, tus_upload.data_type)
    ...
    logger.info(
        "Validation %s for batch %s (run_id=%d)",
