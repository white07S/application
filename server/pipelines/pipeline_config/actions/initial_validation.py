"""Initial validation and parsing for Actions data.

This module handles file validation, CSV parsing, and parquet conversion
for Actions (issue action plans) data.
"""
import json
...
class InitialValidator:
    """Handles file validation, parsing, and parquet conversion for Actions.
    ...
    expected_file_patterns: List[str] = ["actions*.csv", "*.csv"]
    ...
    def validate_file_requirements(self, file_paths: List[Path]) -> None:
        ...
        if len(file_paths) != self.expected_file_count:
            raise InitialValidationError(
                f"Expected {self.expected_file_count} file(s) for actions, got {len(file_paths)}",
    ...
    def parse_csv(self, file_path: Path) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(
                "Parsed actions CSV (enterprise format): {} rows, {} columns from {}",
    ...
    def parse_and_convert(self, file_paths: List[Path]) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        """Parse CSV files and return a DataFrame.
...
def get_validator() -> InitialValidator:
    """Get the validator instance for actions."""
    return validator
